...
//--------------------------------------------------------------------------------
	...
	// Program RAM��CheckSum�̋N��
	data[0] = 0xF0;											//CmdID
	data[1] = 0x0E;											//CmdID
	data[2] = (unsigned char)((ptr->SizeUpdataCode >> 8) & 0x000000FF);	//�������݃f�[�^(MSB)
	data[3] = (unsigned char)(ptr->SizeUpdataCode & 0x000000FF);			//�������݃f�[�^
	data[4] = 0x00;											//�������݃f�[�^
	data[5] = 0x00;											//�������݃f�[�^(LSB)
	...
	do{
		...
		RamRead32A( 0x0088, &UlReadVal );					// PmCheck.ExecFlag�̓ǂݏo��
	}while ( UlReadVal != 0 );
	...
	IOWrite32A( FLASHROM_FLAMODE , 0x00000002);
	// CheckSum�l�̔���(���Ғl�́AHeader��define����Ă���)
	for( i=0; i<8; i++) {
		if(ReadData[7-i] != *p++ ) {  							// CheckSum Code�̔���
...
UINT_8 EraseUserMat128(UINT_8 StartBlock, UINT_8 EndBlock )
{
	...
	for( i=StartBlock ; i<EndBlock ; i++) {
		RamWrite32A( 0xF00A, ( i << 10 ) );				// FromCmd.Addr�̐ݒ�
		RamWrite32A( 0xF00C, 0x00000020 );				// FromCmd.Control�̐ݒ�(�u���b�N����)
...
//			WitTim( 4 );
			...
			RamRead32A( 0xF00C, &UlReadVal );					// FromCmd.Control�̓ǂݏo��
		}while ( UlReadVal != 0 );
	}
	IOWrite32A( 0xE0701C , 0x00000002);
	return(0);
...
UINT_8 ProgramFlash128_LongBurst( DOWNLOAD_TBL_EXT* ptr )
{
	...
	RamWrite32A( 0xF007, 0x00000000 );						// FlashAccess Setup
//	RamWrite32A( 0xF00A, 0x00000000 );						// FromCmd.Addr�̐ݒ�
	RamWrite32A( 0xF00A, 0x00000030 );						// FromCmd.Addr�̐ݒ�
	...
	{
		if( ++UcOddEvn >1 )  	UcOddEvn = 0;	// �����Check
...
UINT_8 ProgramFlash128_Standard( DOWNLOAD_TBL_EXT* ptr )
{
	...
	IOWrite32A( 0xE0701C , 0x00000000);
	RamWrite32A( 0xF007, 0x00000000 );						// FlashAccess Setup
	RamWrite32A( 0xF00A, 0x00000010 );						// FromCmd.Addr�̐ݒ�
	data[0] = 0xF0;						// CmdH
	data[1] = 0x08;						// CmdL
	data[2] = 0x00;						// FromCmd.BufferA�̃A�h���X
...
TRACE("[%d]UcOddEvn= %d , data[1]= %d \n", i, data[1], NcFromVal );
		...
		RamWrite32A( 0xF00B, 0x00000010 );							// FromCmd.Length�̐ݒ�
		UlCnt = 0;
		if (UcOddEvn == 0){
			do{															// �������݂̏I������
				RamRead32A( 0xF00C, &UlReadVal );						// FromCmd.Control�̓ǂݏo��
		 	...
		 	RamWrite32A( 0xF00C, 0x00000004 );	// FromCmd.Control�̐ݒ�(��������)
		}else{
			do{															// �������݂̏I������
				RamRead32A( 0xF00C, &UlReadVal );						// FromCmd.Control�̓ǂݏo��
				if( UlCnt++ > 250 ) {
			...
			RamWrite32A( 0xF00C, 0x00000008 );	// FromCmd.Control�̐ݒ�(��������)
		}			
	}
...
//Infomat�̊m�F�B����Re-Adjust���Ă��Ȃ��Ȃ�Ď��s�B
	ans = CheckDrvOffAdj();
	if( ans == 1 ){
...
//--------------------------------------------------------------------------------
	...
	IOWrite32A( 0xE0701C , 0x00000000);
	RamWrite32A( 0xF00A, 0x00000000 );									// FromCmd.Addr�̐ݒ�
	RamWrite32A( 0xF00D, ptr->SizeFromCodeValid );						// �L��CheckSum�T�C�Y�̐ݒ�

	RamWrite32A( 0xF00C, 0x00000100 );									// FromCmd.Control�̐ݒ�(CheckSum���s)
	WitTim( 6 );
	UlCnt = 0;
	do{																	// �������݂̏I������
		RamRead32A( 0xF00C, &UlReadVal );								// FromCmd.Control�̓ǂݏo��
	...
	RamRead32A( 0xF00D, &UlReadVal );									// CheckSum�l�̓ǂݏo��
...
UINT_8	FlashBlockErase( UINT_8 SelMat , UINT_32 SetAddress )
{
	...
	if( SelMat != USER_MAT && SelMat != INF_MAT0 && SelMat != INF_MAT1 && SelMat != INF_MAT2  )	return 10;	// INF_MAT2��Access���Ȃ�
...
UINT_8	FlashMultiRead( UINT_8 SelMat, UINT_32 UlAddress, UINT_32 *PulData , UINT_8 UcLength )
{
	...
	// fail safe
	// reject irregular mat
	if( SelMat != USER_MAT && SelMat != INF_MAT0 && SelMat != INF_MAT1 && SelMat != INF_MAT2  )	return 10;	// INF_MAT2��Read only Access���Ȃ�
	// reject command if address inner NVR3
	if( UlAddress > 0x000003FF )											return 9;
...
UINT_8	FlashBlockWrite( UINT_8 SelMat , UINT_32 SetAddress , UINT_32 *PulData)
{
	...
	// fail safe
	// reject irregular mat
//	if( SelMat != INF_MAT0 && SelMat != INF_MAT1  )			return 10;	// USR MAT,INF_MAT2��Access���Ȃ�
	if( SelMat != INF_MAT0 && SelMat != INF_MAT1 && SelMat != INF_MAT2  )			return 10;	// USR MAT��Access���Ȃ�
	// 
	if( SetAddress > 0x000003FF )							return 9;
	...
	IOWrite32A( FLASHROM_FLA_ADR, ((UINT_32)SelMat << 16) | ( SetAddress & 0x000010 )) ;// address��page�̂ݎw��
	// page write Start
	IOWrite32A( FLASHROM_FLAMODE , 0x00000000);
...
TRACE( "WrHallCalData : Mode = %d\n", UcMode);
	/* Back up ******************************************************/
	ans =FlashMultiRead( INF_MAT0, 0, UlMAT0, 32 );	// check sum �ȊO
	if( ans )	return( 1 );
...
/*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/
/* function name    : lstsq								 							  	 */
/* input parameter  : double x[] X���̔z��f�[�^                                         */
/*                    double y[] Y���̔z��f�[�^                                         */
/*                    int n      �f�[�^��                                              */
/*                    int m      �ߎ�����������                                          */
/* output parameter : double c[] �ߎ������e���̌W���z��                                  */
/*                    �߂�l�́A0�Ő���A-1�Ŏ������s                                    */
/* comment          : �ŏ�2��ߎ��@                                                      */
/*				   	   	    			      								  2018.03.07 */
/*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/
...
//	RamWrite32A( 0xF007, 0x00000000 );						// FlashAccess Setup
	...
	do{
		...
		RamRead32A( 0x5005, &UlReadVal );					// complite Flag�̓ǂݏo��
	}while ( UlReadVal != 0 );
	...
	do{
		...
		RamRead32A( 0x5006, &UlReadVal );					// complite Flag�̓ǂݏo��
	}while ( UlReadVal != 0 );
	IOWrite32A( 0xE0701C , 0x00000002);
